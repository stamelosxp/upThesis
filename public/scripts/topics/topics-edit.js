...
function checkFormState(topicId, formInputs, submitButton) {
    const anyModified = formInputs.filter(i => i.classList.contains('modified'));

    const allValid = formInputs.every(input => {
        return validateInput(input, topicId);
    });

    if (submitButton) {
        if (anyModified.length > 0 && allValid) {
...
function changeInputsNew(newTopicItem) {
    ...
    const topicsInputs = Array.from(newTopicItem.querySelectorAll('input, textarea'))
        .filter(formInput => {
        ...
        input.addEventListener(eventType, (e) => {
            if (input.type === 'file') {
                ...
                if (fileInput.files && fileInput.files.length > 0) {
                    ...
                    fileInput.classList.add('modified');
                } else {
                    fileInput.classList.remove('modified');
                }
            } else {
...
function initEditFormTracking(topicItem) {
    if (!topicItem) return;
    const expandedItem = topicItem.querySelector('.topic-item-expanded.edit-mode');
    if (!expandedItem) return;

    let topicId = expandedItem.dataset.topicId;

    const formInputs = Array.from(expandedItem.querySelectorAll('input, textarea'))
        .filter(formInput => {
            return formInput.id.includes(topicId);
        });
...
function cancelNewTopic() {
    ...
    if (newTopicItem) {
        ...
        newTopicItem.querySelectorAll('input[type="text"], input[type="file"], textarea').forEach(input => {
            ...
            const label = newTopicItem.querySelector('label[for="' + input.id + '"]');
...
function uploadNewTopic() {
    try {
        ...
        const titleInput = newTopicItem.querySelector('input[id^="topic-input-title-"]');
        const descInput = newTopicItem.querySelector('textarea[id^="topic-input-desc-"]');
        ...
        uploadNewTopicRequest(formData).then(result => {
            if (result && result.success) {
                const topicsList = document.getElementById('topics-list');
                if (topicsList) {
                    const topicLiItem = document.createElement('li');
                    topicLiItem.className = 'topic-item';
                    const collapsedContent = renderCollapsedTopic(result.topic, 'new');
                    const expandedContent = renderExpandedTopic(result.topic, 'new', userRole);
        ...
        (error) {
        const message = 'Υπήρξε σφάλμα κατά τη δημιουργία του νέου θέματος. Προσπαθήστε ξανά.';
...
function exitEditMode(topicItem) {
    ...
    expandedItem.querySelectorAll('input[type="text"], textarea').forEach(input => {
    ...
    try {
        ...
        deleteTopicRequest(topicId).then(result => {
            if (result.success) {
                ...
                const message = 'Υπήρξε σφάλμα κατά τη διαγραφή του θέματος. Προσπαθήστε ξανά.';
                alertPopUP(message, 'red');
            }
        });
    } catch (error) {
...
function updateTopic(topicItem) {
    try {
        ...
        const titleInput = expandedItem.querySelector('input[id^="topic-input-title-"]');
        const descInput = expandedItem.querySelector('textarea[id^="topic-input-desc-"]');
        ...
        updateTopicRequest(topicId, formData).then(result => {
            if (result.success) {
                if (!assignmentInput.dataset.studentAssignedId) {
                    topicItem.innerHTML = '';
                    topicItem.appendChild(renderCollapsedTopic(result.topic, updateString));
                    topicItem.appendChild(renderExpandedTopic(result.topic, updateString, userRole));
                    alertPopUP(result.message, 'green');
                    updateButtonListeners();
                    sortTopicsUI(topicItem);
                } else {
                    const message = 'Το θέμα ανατέθηκε επιτυχώς στον/στην  ' + assignmentInput.value;
        ...
        (error) {
        const message = 'Υπήρξε σφάλμα κατά την ενημέρωση του θέματος. Προσπαθήστε ξανά.';
        alertPopUP(message, 'red');
    }
}
...
function openAssignModal(modalContainer, enteredTopicId) {
    ...
    if (innerModal) {
        ...
        fetchAvailableStudents('', signal).then((data) => {
    ...
    if (closeModalBtn && !closeModalBtn.dataset.bound) {
        ...
        closeModalBtn.addEventListener('click', () => closeAssignModal(modalContainer));
    }


    const assignBtn = modalContainer.querySelector('.modal-footer .accept-button');
    if (assignBtn && !assignBtn.dataset.bound) {
        ...
        assignBtn.addEventListener('click', () => {
            ...
            const assignmentInput = document.querySelector(`.topic-item-expanded[data-topic-id="${topicId}"] .assignment`);
...
async function fetchAvailableStudents(inputValue, signal) {
    try {
        const response = await fetch(`/api/students/available?q=${encodeURIComponent(inputValue)}`, {signal});
...
function toggleFunction() {
    ...
    topicItems.forEach(function (topicItem) {
        ...
        toggleExpandCollapseView(collapsedContent, collapseButton, expandedContent, expandButton, editMode);
...
function sortTopicsUI(topicItem) {
    ...
    items.sort((a, b) => {
        const titleA = a.querySelector('.topic-item-collapsed .topic-header')?.textContent.trim().toLowerCase() || '';
        const titleB = b.querySelector('.topic-item-collapsed .topic-header')?.textContent.trim().toLowerCase() || '';
...
export function updateButtonListeners() {
    ...
    document.querySelectorAll('.delete-topic-button').forEach(deleteButton => {
        ...
        btn.addEventListener('click', () => {
            const topicItem = btn.closest('.topic-item');
            if (topicItem) {
                const confirmed = confirm('Είστε σίγουροι ότι θέλετε να διαγράψετε αυτό το θέμα; Αυτή η ενέργεια δεν μπορεί να αναιρεθεί.');
    ...
    // Remove existing file listener (delegated, no clone needed)
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            document.querySelectorAll('.modal-container.active').forEach(container => closeAssignModal(container));
        }
    });
}