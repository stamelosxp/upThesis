async function fetchDashboardStats(professorId) {
    try {
        const response = await fetch(`/api/dashboard/${professorId}`);
        if (!response.ok) {
        ...
        if (invitesCountEl) invitesCountEl.textContent = stats.totalPendingInvitations;
        if (supervisionsCountEl) supervisionsCountEl.textContent = stats.totalThesesSupervising;
...
function cycleDashboardStats() {
    ...
    let activeIndex = Array.from(statsItems).findIndex(item => item.classList.contains('active'));
